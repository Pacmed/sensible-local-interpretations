...
def contrib_score(word: str, review: str, tokenizer, clf, masked_predictor, device='cuda'):
    ...
    Returns
    -------
    contrib_score_iid, contrib_score_conditional
    '''
    # original prediction
    ...
    n = 1000 # there are 30522, we are only going to use the top ones
    ...
    return pred_orig - ave, pred_orig - ave_weighted
...
for i in tqdm(range(num_reviews)):
    ...
    for word in review.split():
        ...
        if not word in [';', ',', '.', '!', '?', '/']:
            ...
            try:
                contrib_score_iid, contrib_score_conditional = \
                contrib_score(word, review, tokenizer, clf, masked_predictor, device)

                if not word in scores_iid:
                    scores_iid[word] = [contrib_score_iid]
                    scores_conditional[word] = [scores_conditional]
                else:
                    scores_iid[word].append(contrib_score_iid)
                    scores_conditional[word].append(contrib_score_conditional)
            except:
                pass
            
    if i % save_freq == 0:
        pkl.dump({'iid': scores_iid, 'conditional': scores_conditional}, open(f'scores_{i}.pkl', 'wb'))